<template>
  <DataTable :columns="columns" :data="data" @add="showAdd" @edit="showEdit" @delete="showDelete">
    <template #search>
      <input v-model="search" class="border rounded px-3 py-1" placeholder="搜索环节..." />
    </template>
  </DataTable>
  <EditDialog :visible="dialogVisible" title="添加/编辑环节" :onSubmit="handleSubmit" :onCancel="() => dialogVisible=false">
    <input v-model="form.name" class="border rounded px-3 py-2 w-full mb-3" placeholder="环节名称" />
...
const columns = [ { title: '环节名称', key: 'name' }, { title: '状态', key: 'status' }, { title: '备注', key: 'remark' } ]
